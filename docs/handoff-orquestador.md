...
- **Backlog técnico/DEVEX: el DEVEX está esencialmente cerrado, pero hay activos de producto/bug.** Cerrados en esta etapa: T-109, T-111 (F1+F2+F2b), **T-112** (E2E ya corren aislados contra Supabase local — el job `E2E (Supabase local)` es required check; se removió el `if: false`), F1.2/F1.3, T-113a/T-113b/T-113d, Dependabot, T-082-FU + FU5. Activos nuevos en operativo.md: T-115 (hardening billing), T-076 (doc drift src/modules), **T-120** (lifecycle CAPAs), **T-121** (dormido: coherencia `consultora_id` denormalizado), y 2 follow-ups de checklists (guard redirect tombstone→original; anularEjecucionAction valide estado='cerrada').
- **Próximo:** **Tanda de consistencia EPP↔calendario COMPLETA y en prod** (T-114 · T-117/FU1 asistente IA · T-119 lifecycle planificaciones · T-118 sync calendario→dominio). Tramo ahora: **T-120** (lifecycle CAPAs) · **RGRL completo** · o siguiente módulo, a prioridad del owner. Pendiente abierto **GPS** sigue (ver abajo).
