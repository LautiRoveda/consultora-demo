...
- **Próximo:** TODA la tanda de consistencia (T-114/117/FU1/119/118) Y la auditoría de integridad ADR-0015 (T-122/120/123/124/121) ✅ EN PROD. La deuda estructural de integridad está cerrada operativamente. **PRÓXIMO HILO A RETOMAR**: pulido del asistente IA de EPP (T-117) que quedó a medias — (a) los `**` markdown crudos en la UI del chat (render markdown en `asistente-client.tsx`) y (b) el testeo del chat que el owner dejó sin terminar. Después: RGRL completo (contenido del matriculado) / siguiente módulo / los FU dormidos (T-117-FU2, T-121-FU, flaky E2E, doc-drift data-model). Pendiente abierto **GPS** sigue (ver abajo). _Sesión 2026-06-05: tanda de integridad completa (auditoría ADR-0015 → 5 tickets en prod); próximo chat: retomar el LLM del asistente._
- **Pendiente abierto:** GPS "Usar mi ubicación" en el cierre de inspección (CerrarInspeccionForm) falla con mensaje de permisos. Diagnóstico provisorio: navigator.geolocation exige HTTPS (secure context) — en dev local http://IP el browser lo bloquea y lo reporta como "permiso denegado" sin prompt; en prod (https) debería andar con permiso de sitio+SO. Falta que el owner confirme la URL del celular. GPS es opcional → NO bloquea el cierre.
