...
- **Backlog técnico/DEVEX: el DEVEX está esencialmente cerrado, pero hay activos de producto/bug.** Cerrados en esta etapa: T-109, T-111 (F1+F2+F2b), **T-112** (E2E ya corren aislados contra Supabase local — el job `E2E (Supabase local)` es required check; se removió el `if: false`), F1.2/F1.3, T-113a/T-113b/T-113d, Dependabot, T-082-FU + FU5, **auditoría de integridad ADR-0015** (T-122/120/123/124/121, 2026-06-05), **campo Puesto → catálogo** (T-128 + T-129 fase A/B en prod: selector del catálogo + corte de consumers + drop de la columna legacy, #231/#232/#234), **rediseño del dashboard** (T-131 fase A operativo #235 + fase B semáforo por cliente #238) y **T-132** (#236, endureció el flake E2E del guard `EXEC_NOT_DRAFT`). Activos nuevos en operativo.md: T-115 (hardening billing), T-076 (doc drift src/modules), T-117-FU2 (dormido), T-121-FU (dormido: coherencia Ring B/C), flaky E2E (`checklists-ejecuciones.spec.ts:100`), doc-drift data-model (`03-data-model.md` stale), T-113c (dormido), **FU dormidos nuevos**: infra-flake del `supabase start` (502 del edge-runtime en CI → la suite no corre; re-run lo salva) · drift Zod↔SQL en DNI (la regex acepta 9-12 dígitos vs el CHECK 7-8 → error genérico) · E2E de fase B del dashboard (semáforo, opcional) · `node-version: 22` es pin major-only y lint-staged v17 exige ≥22.22.1. Las 2 follow-ups de checklists (guard redirect tombstone→original; anularEjecucionAction valide estado='cerrada') siguen documentadas en la sección "Checklists · follow-ups abiertos" de operativo.md.
- **Próximo:** El **asistente IA de EPP (T-117)** cerró (streaming SSE + render markdown en `asistente-client.tsx` + persistencia del chat: T-117-FU3/T-125/T-126 en prod) y el **responsive T-127** quedó en prod (tandas 1-6 + follow-ups; el guard anti-drift `QUICK_LINKS`↔`NAV_ITEMS` de la Tanda 7 ya está, #230 — queda solo el pulido de tipografía/densidad). La deuda estructural de integridad (ADR-0015: T-122/120/123/124/121) sigue cerrada operativamente. Las dos épicas recientes quedaron **cerradas**: **campo Puesto → catálogo** (T-128 + T-129 fase A/B, drop de la columna legacy en prod) y **rediseño del dashboard** (T-131 fase A operativo + fase B semáforo por cliente), más **T-132** (endurecimiento del flake E2E del guard `EXEC_NOT_DRAFT`). **PRÓXIMO (estado 2026-06-11)**: cerradas en prod desde el último handoff — **auditoría de seguridad Opus 4.8** (T-133 M-1+L-1 / T-134 L-4 / T-135 L-2+L-3 / T-136 doc-sync; la llevó un chat orquestador aparte) · **T-137** (skew `db:types` → `scripts/gen-types.mjs` stripea el bloque `__InternalSupabase`; ver lessons) · **épica templates moldeables** (T-138 fase 1 campos personalizados + instrucciones + fase 2 secciones configurables · T-139 fase 3 plantillas guardables, tabla `informe_plantillas` + RLS per-consultora) · **épica editor WYSIWYG del informe** (T-140 Fase 1 editor Plate `platejs` + source-mode + round-trip markdown remark-gfm + FU1 responsive/full-width · T-141 Fase A toolbar de formato + operaciones de tabla por celda · Fase C autosave server draft-only con columna `informes.contenido_borrador` + promoción del borrador al publicar) · **épica onboarding** (T-142 wizard + welcome email · FU1 el onboarding se marca en `createInformeAction`/`createEntregaAction` —la acción real, no el click— vía `markOnboardingCompletedIfPending` service-role idempotente · FU2 banner compacto colapsable, cookie `onboarding_collapsed`; migración `20260611000002` aplicada pre-merge porque `getCurrentConsultora` la lee en cada request). **Diferidos/dormidos**: T-141 B1 (slash commands — decisión de dep `@ariakit/react` vs hand-roll) y B2 (syntax highlighting `lowlight`), bajo valor para el público no técnico. **Cerrada en prod (2026-06-14): épica RAR COMPLETA** (T-143..T-147, #259/#261/#262/#263/#264) — vertical nuevo `src/app/(app)/rar/`, la DJ anual del 658/96 de trabajadores expuestos: catálogo de agentes 658/96 `rar_agentes` (seed de 22 agentes con códigos ESOP de la Res SRT 81/2019) + exposición por **establecimiento** `cliente_puesto_agentes` (FK compuestas Ring A; T-145 dropeó la `puesto_agentes` puesto-global, **ADR-0016 decisión B revisada**) + planilla PDF NTE/DAR Res SRT 37/2010 (T-144) + vencimiento anual en el calendario (T-146: `rar_presentaciones` inmutable con snapshot legal + tipo `rar_anual` + RPC `gen_rar_vencimiento_calendar_for`, reminders [60,30,7,0]) + semáforo del cliente cuando el RAR vence + historial de presentaciones + descarga histórica del PDF desde el snapshot (T-147, 4ª rama `rar_anual` en `semaforo_clientes` por CREATE OR REPLACE de firma idéntica). Migraciones `20260613000001..04` aplicadas a prod ANTES del merge (gate migración-primero); decisiones de modelo en **ADR-0016**. **PRÓXIMO a definir por el owner**: informes de medición con estructura propia (ruido/iluminación/PAT/carga de fuego — depende del feedback de un amigo HyS sobre un PDF de demo) / auditoría de seguridad Opus 4.8 / T-115 hardening billing / FU nuevos (DRY del pipeline PDF de los 5 routes; validación legal de los códigos ESOP + texto de la DJ por el matriculado) / los FU dormidos. _El go-live de pagos REALES (Mercado Pago test→prod) lo lleva un chat orquestador aparte._
...
- **El orquestador corre `git` read-only desde su sandbox para diagnóstico general** (`git log`, `git diff` del working tree); ya no hay junction que lo bloquee. **PERO el mount Windows→sandbox da vistas inconsistentes de los internals de `.git`** (HEAD, refs, locks) — puede reportar un `.git/HEAD` truncado o un lock fantasma que en Windows está sano. Ante cualquier señal de "corrupción" / HEAD irresoluble desde el sandbox, **confirmar con CC en terminal nativa ANTES de alarmar o tocar nada** (`type .git\HEAD`, `git fsck`). `gh` no está en el sandbox → PRs / CI / merge los corre Lautaro o el CC de Antigravity. **Esta trampa fue MUY recurrente (sesión 2026-06-11): `git` desde el sandbox tiró `bad config line N in .git/config`, una branch nueva vista como "does not have any commits yet", y `git diff` con archivos truncados a mitad de palabra — TODO vista del mount, NO corrupción (el CI verde + el working tree real lo desmienten). Clave: los file-tools (Read/Grep) leen el working tree REAL y SON confiables — el que falla es `git` (que lee `.git/`). Workaround para el OK de merge cuando el mount bloquea la verificación por hash: pedir a CC el output NATIVO (`git log --stat main..<branch>` + `git log --all --name-only | grep` de archivos ajenos como env.ts/dni/.xlsx) para confirmar que el commit entra limpio antes de aprobar el merge.**
...
- **El hook `commit-msg` (`.husky/commit-msg`) acepta SOLO dos formatos de subject:** Formato A `T-XXX · <desc>` (con el ticket numérico y `·`, descripción ≥10 chars) **o** Formato B `<prefijo>: <desc>` con prefijo en `docs|fix|chore|refactor|test|style|build|ci|perf|revert`. **`feat` NO está en la lista → `feat: …` rebota.** Para commits de ticket usá siempre Formato A (`T-XXX · …`); el Formato B queda para cambios sin ticket (doc-sync, chores). Recordá además el gotcha UTF-8 del `·` en el subject (commitear con `git commit -F archivo` en UTF-8, o evitar el `·` usando Formato B).
