...
### Defecto conceptual de modelo en una fase temprana: corregir ANTES de apilar fases encima

**Origen**: T-145 (épica RAR). La Fase 1 (T-143) modeló la exposición a agentes de riesgo como `puesto_agentes` (puesto×agente **global** al tenant), tomándolo como una simplificación válida del MVP. Pero el RAR es una **DJ por establecimiento**: dos clientes con el mismo puesto "Soldador" pueden tener exposiciones distintas, y la planilla declara por establecimiento. El modelo puesto-global no era una simplificación legítima sino un **defecto conceptual en el corazón del feature** — habría producido nóminas incorrectas en cuanto un tenant tuviera ≥2 clientes con puestos homónimos. Se corrigió en T-145 (`cliente_puesto_agentes`, cliente×puesto×agente) **antes** de construir la planilla (Fase 2) y el vencimiento (Fase 3) encima. Como el feature era nuevo y no había datos reales, el costo fue un `drop`+`create` (0 filas en prod) + la revisión de ADR-0016 (decisión B). **Regla**: cuando una "simplificación" de una fase temprana resulta ser un defecto conceptual del dominio (no una omisión de alcance), corregila apenas se detecta y antes de apilar fases que la asuman — es barata sin datos reales y carísima después (migración de datos + reescritura de los consumers). Distinguir "alcance recortado a propósito" (legítimo) de "el modelo no representa el dominio" (deuda que crece exponencialmente).
...
### Skew PostgREST local↔prod en `pnpm db:types` — RESUELTO (T-137)

**Origen**: T-126. **Resuelto**: T-137. Prod corre PostgREST 14.5; la imagen de Supabase local (`supabase start`) es 14.x → al regenerar `src/shared/supabase/types.ts` con `pnpm db:types`, `gen types --linked` **reintroducía** el bloque `__InternalSupabase` (`PostgrestVersion`) que el gate de drift de CI (`gen types --local` + `git diff`) rechaza, obligando al hand-edit del archivo. **Fix (T-137)**: `db:types` y el gate ahora comparten `scripts/gen-types.mjs`, que stripea ese bloque con la misma normalización en ambos lados (dev `--linked` sin Docker, CI `--local` sin secrets) → idempotentes. El bloque es info de entorno, no de schema (el footer ya hace `Omit<Database, '__InternalSupabase'>` → borrarlo es no-op a nivel de tipos). Se descartó bumpear la imagen local: la versión de un PostgREST de prod managed es un blanco móvil.

### Objeto DB nuevo en una migración rompe el drift gate de `types.ts`

**Origen**: T-129 fase A (commit `9f6663b`). **Relacionado**: skew PostgREST ↑ (ya resuelto en T-137).

Una migración que agrega un objeto visible a los tipos generados (función/RPC, tabla, columna, enum) rompe el gate `types.ts sin drift` del job Integration **aunque el PR difiera `db:types` a propósito** y no toque columnas existentes: `gen types --local` (que corre el gate) lo incluye y el `git diff` contra el `types.ts` commiteado falla. **Fix sin Docker**: NO correr `pnpm db:types` (= `gen types --linked` contra prod) — prod **todavía no tiene la migración** (el `db push` va después del merge), así que `--linked` generaría tipos SIN el objeto nuevo. En cambio, leer el diff exacto que imprime el gate (`gh run view --job <id> --log-failed`) y aplicar **a mano** SOLO ese bloque al `types.ts` (para una función `(p_x uuid default null) returns jsonb`: 4 líneas en `Functions:`, `Args: { p_x?: string }` + `Returns: Json`). Es la misma adición que produce el gen local → el gate pasa, sin tocar el resto. Post-T-137 ya no hay ruido del bloque `__InternalSupabase` en ese diff (el script `gen-types.mjs` lo stripea), así que el diff a mano es limpio.
