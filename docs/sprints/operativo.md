...
## T-138 ✅ Templates moldeables: personalización de informes (fases 1+2)

Cualquier higienista moldea sus informes a su laburo sin tocar código. Todo vive en `informe_metadata.data` (jsonb) → SIN migración; campos nuevos todos `.optional()` → metadata pre-T-138 parsea igual y un informe sin personalización produce un user message byte-idéntico al de antes. Fase 3 (plantillas guardables, tabla+RLS) → **T-139 ✅** (cierre de la épica).

- **Fase 1 — campos personalizados + instrucciones (LOS 5 TIPOS)**: `campos_personalizados: [{label,valor}]` (cap 10; 60/500 chars) + `instrucciones_adicionales` (cap 1500), factories compartidas en `templates/common/campos-extra.ts`. Render (`common/render-extra.ts`): bloques sanitizados (`sanitizeField`; valores colapsados a una línea; instrucciones blockquoteadas) SIEMPRE antes del footer de re-anclaje — la última palabra la tiene el sistema. UI: `PersonalizacionSection` colapsable compartida (slot para fase 2) + `useFieldArray` (primer uso en templates). Los 5 system prompts ganan el bullet estático "Preferencias del consultor (NO son reglas)". `EditorView` mergea defaults para metadata vieja. PR #247, merge `7e92bd0`.
- **Fase 2 — secciones configurables (SOLO tipos sin estructura legal: relevamiento + capacitacion + otros)**: elegir qué secciones lleva el informe, en qué orden, + secciones custom. Catálogo client-safe `{id,label}` en `templates/{tipo}/secciones.ts`; los CUERPOS markdown quedan server-side en `ai/prompts/{tipo}.ts` (`CUERPO_BY_SECCION`, Record exhaustivo por id → drift catálogo↔cuerpos = error de compilación). El system prompt se re-arma en module-load desde el catálogo (string estático → **prompt caching intacto**) + regla condicional "Estructura solicitada"; la selección viaja en el user message. **rgrl y accidente quedan FIJOS** (estructura legal SRT) — decisión de diseño, no limitación. En `otros`, la regla de "4 obligatorias" se suspende solo cuando hay estructura solicitada. `normalizeSecciones` dropea config==default (jsonb lean). UI: `SeccionesConfigField` (useFieldArray.move, sin RPC — a diferencia del reorder de checklists) + hoist de `ReorderButtons` a `shared/ui/reorder-buttons.tsx`.
- **Seguridad (red→green)**: `templates-seguridad-injection.test.ts` se escribió ANTES de implementar (corrida roja documentada en el PR de fase 1) — payloads de inyección en instrucciones / campos / título+descripción de sección custom quedan inertes: sanitizados, blockquoteados/inline, sin headings crudos, footer después. Integration asserta `system[0].text === SYSTEM_PROMPT_<TIPO>` byte a byte (compliance + cache), incluso con bloque SRT en `system[1]`.
- **Canary de no-drift del refactor de prompts**: fixtures byte-exactas del bloque `# Estructura` capturadas ANTES del refactor (`scripts/dev-capture-estructura-fixtures.ts` → `tests/unit/fixtures/estructura-{tipo}.md`); `prompts-secciones-assembly.test.ts` corre verde como baseline y sigue verde post-refactor (un byte de drift lo rompe).
- Tests: 1038 unit+component en verde; 9 casos integration nuevos (CI). Gotcha nuevo documentado en 07-zod-rhf-gotchas (colisión `id` de useFieldArray → `seccion_id`).

## T-139 ✅ Templates moldeables fase 3: plantillas guardables ("Mis plantillas") — CIERRE DE ÉPICA

El higienista guarda una configuración de personalización (fases 1+2) como plantilla reutilizable per-consultora ("Mi relevamiento de ruido") y la aplica al crear/editar informes. Primera fase de la épica que toca la DB. Cierra templates moldeables (T-138 fases 1+2 + T-139 fase 3).

- **Tabla `informe_plantillas`** (migración `20260610000001`, aplicada a prod pre-merge con diff-validado + OK del owner): `tipo` espeja el check de `informes.tipo`, `nombre` 1..80, `config jsonb` (check estructural mínimo `jsonb_typeof='object'` — el shape per-tipo es de Zod, no de SQL), soft-delete `archived_at`, **unique parcial `(consultora_id, tipo, lower(nombre)) where archived_at is null`** (archivar libera el nombre), índice de lista calzado al orden `(tipo, nombre)`.
- **RLS PER-CONSULTORA** (fork a, decisión owner — lo OPUESTO al chat T-126): las plantillas son activos del negocio; cualquier member las ve/aplica/edita (`is_member_of_consultora`, sin filtro `user_id`). `created_by` es atribución (nullable, `on delete set null`, validado `= auth.uid()` en el insert — sin spoof). DELETE default-deny. **Sin audit** (fork b: preset UX, no dominio legal — el artefacto con valor legal es el informe, donde la config queda copiada). **Sin billing gate** (decisión owner: se gatea donde hay costo/valor de IA; un trial vencido ordena sus presets y los tiene listos al reactivar).
- **Validación de config** (`templates/registry/plantilla-config.ts`): schemas **`.strict()`** per-tipo armados 100% con los factories de fase 1+2 (campos/instrucciones/`seccionesField(ids del tipo)`) → rechaza datos del cliente (config es ESTRUCTURA, nunca `razon_social`/`cuit`), secciones de otro tipo, `secciones` en rgrl/accidente y over-cap, con cero validación nueva. Rechazo de "plantilla vacía" POST-normalize (caza "solo secciones en orden default", que parsea pero normaliza a no-op).
- **Snapshot-on-apply** (fork d): aplicar COPIA la config a los values del form → persiste por el flujo existente de informes (re-valida `<tipo>MetadataSchema`); sin FK informe→plantilla; editar/archivar la plantilla nunca toca informes creados. Sanitización heredada: la config aplicada pasa por `render-extra` al generar — sin superficie nueva de injection. **`degradePlantillaConfig`** re-valida al aplicar: plantilla vieja (secciones que ya no existen, caps achicados) se degrada filtrando/recortando — nunca rompe; insalvable → "plantilla incompatible".
- **UI**: `PlantillaControls` montado A NIVEL PÁGINA (wizard step 2 + collapsible de metadata del editor) — la firma `{form, disabled}` del registry y los 5 MetadataForm quedan intactos. Aplicar sin `secciones` en la config resetea la estructura al default del catálogo. Gestión en `informes/plantillas` (fork c: en informes, NO en settings — settings es config de la consultora, esto es config de informes): lista agrupada por tipo, renombrar, archivar; sin alta (las plantillas nacen del form). Link en el header de `/informes`.
- **Tests**: 14 unit de config strict/normalize/degrade + 7 component de aplicar/guardar + 11 integration RLS (caso estrella per-consultora: a2 del MISMO tenant SÍ ve y SÍ edita — invierte el molde del chat; cross-tenant/spoof `created_by`/anon 42501/DELETE-deny/CHECKs 23514/unique 23505 + liberación por archive). Gotcha del unique case-insensitive: test con variación de case solo-ASCII (el fold de `lower('Ó')` depende del locale — ver lessons).
- **FU**: test de componente de `PlantillasView` (rename/archive) — la lógica server ya está cubierta por unit+integration.
...
## T-142 ✅ Onboarding wizard + welcome email (+ FU1 trigger real + FU2 banner compacto) — EN PROD

Primer paso de activación/conversión pre-revenue: el consultor que confirma su cuenta recibe un welcome email y, ya dentro, un wizard que lo guía a su primer valor real (crear un cliente → cargar un informe o una entrega de EPP). Cierra en 3 commits: el ticket base, FU1 (el onboarding se marca completo en la acción de valor, no en el click) y FU2 (banner compacto no intrusivo).

- **T-142 (`2349e84`) · wizard + welcome email**: el **welcome email** usa un template nuevo `welcome.ts` (calcado del patrón de `trial-expires`) con el sender genérico `sendEmail` de `senders/email.ts`. Se dispara con `after()` en `/auth/callback` gateado por `from=signup` (post-confirmación del mail, NO en `signupAction`) → fire-and-forget, idempotente por el token single-use del callback. Migración `20260611000002_t142_onboarding.sql` (`ALTER consultoras ADD onboarding_completado_at TIMESTAMPTZ`) **aplicada a prod ANTES del merge** (orden código-depende-de-migración: `getCurrentConsultora` la SELECTea en `(app)/layout.tsx` en cada request → si la columna no existe, rompe todo el shell autenticado). El wizard es `OnboardingBanner` (server) + `OnboardingWizard`, gateado por `onboarding_completado_at NULL` + `hasCliente` (`countClientesActivos`). La acción de cierre original (`completeOnboardingAction`) usaba client **autenticado** (no service-role) vía la policy `consultoras_update_own_owner`.
- **FU1 (`f8e109f`) · trigger en la acción real**: el click del wizard ya **no** completa el onboarding (medía intención, no valor). En su lugar `markOnboardingCompletedIfPending` (service-role, best-effort `void`, idempotente con `.is('onboarding_completado_at', null)`) se invoca desde `createInformeAction` y `createEntregaAction` tras la creación exitosa → el onboarding se marca cuando el consultor produce su primer artefacto real. El wizard pasa a server component de solo-navegación; baja de `completeOnboardingAction`/Schema, ahora muertos. **Sin migración.**
- **FU2 (`0ac0ae6`) · banner compacto no intrusivo**: rediseño a un banner compacto y colapsable (research NN/G, Appcues, SIDP): header + barra de progreso "N de 2" siempre visibles, pasos colapsables, botones compactos. El estado de colapso se persiste en la cookie `onboarding_collapsed`, leída por el server para evitar flash de contenido. El wizard pasa a client component. **Sin migración, sin server action.**

## T-143 ✅ RAR Fase 1 — catálogo de agentes 658/96 + modelo de exposición — EN PROD

Arranque de la **épica RAR** (Relevamiento de Agentes de Riesgo): la DJ anual que el empleador presenta a su ART declarando trabajadores expuestos a agentes de riesgo (Dto 658/96 + Res SRT 37/2010). **Fase 1 = catálogo de agentes + modelo de exposición puesto×agente.** NO incluye nómina/planilla PDF (Fase 2) ni vencimiento anual en el calendario (Fase 3) → la épica sigue ABIERTA. Decisiones de modelo fijadas en **[ADR-0016](../adr/0016-rar-modelo-datos.md)** (cliente=establecimiento en MVP · exposición por puesto · catálogo propio ≠ `AGENTES_HYS`). Squash `94b9241` (#259); migración `20260613000001` aplicada a prod ANTES del merge (el código de Fase 1 usa las tablas) + smoke OK. 5 commits separados (migración / seed+actions/queries/schema / UI / tests / fix de orden del test Ring A).

- **Datos**: enum `agente_riesgo_tipo` (`fisico|quimico|biologico|ergonomico`) + tabla `rar_agentes` (catálogo per-consultora, soft-delete `archived_at`, `codigo` natural key del seed) + junction `puesto_agentes` (M:N puesto×agente, **FK COMPUESTAS Ring A** ADR-0015: `(puesto_id,consultora_id)→puestos` y `(agente_id,consultora_id)→rar_agentes`). `set_updated_at` en `rar_agentes`, audit triggers (catálogo created/updated/deleted; junction INSERT/DELETE), RLS helpers T-015 (catálogo SELECT/INSERT/UPDATE sin DELETE; junction SELECT/INSERT/DELETE sin UPDATE). `types.ts` editado a mano (enum + 2 tablas) para no despertar el skew PostgREST de `db:types` (T-137).
- **Seed**: `seedDefaultCatalogAction` owner-only idempotente (pre-check por `codigo`, sin ON CONFLICT) siembra **22 agentes** con los **códigos ESOP reales** de la **Res SRT 81/2019, Anexo III** (ej. `90001` Ruido, `40153` Sílice cristalina), curados por sector (D03). El seed NO asigna agentes a puestos. La fidelidad legal de los códigos la valida el matriculado; el test-meta solo chequea consistencia interna.
...
## T-148 · DEVEX · DRY del pipeline PDF (unificar los 5 routes) ✅

Los 5 routes de PDF (informes / epp / checklists / rar-planilla / rar-historica) repetían el mismo bloque frágil: internal fetch al print page + `x-internal-pdf-render` + AbortController 20s + `injectBaseHref` + `htmlToPdf` + mapeo de errores 500/504. Eran 5 copias del bloque más propenso a bugs del repo. **Hecho**: helper "thick" `renderPrintPageToPdf({ request, printPath, recurso, logPrefix, logBase })` en `@/shared/pdf/render-print-page` — hace la orquestación, loggea con el prefijo/contexto del caller y devuelve `{ ok, pdf }` o la `Response` 500/504 lista; más `pdfDownloadResponse({ pdf, filename })` para la Response 200 común. Los 5 routes migrados (−364 líneas netas); auth/billing/load-entity/validaciones/audit/`logger.info` de éxito quedan per-route. Comportamiento **byte-idéntico** (status/code/mensajes/headers/filename/audit/payloads de log) — los 6 integration tests de PDF pasan sin tocar asserts. Guard anti-reincidencia `pdf-pipeline-guard.test.ts` (tier unit): falla si un route reimplementa el pipeline inline, obligando al 6º route futuro a usar el helper. Sin migración (no toca prod-DB). Era el disparador del FU diferido en T-147.
