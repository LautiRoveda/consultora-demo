...
Última actualización: 2026-06-14 (doc-sync: **épica RAR COMPLETA y CERRADA en prod** —T-143 catálogo + exposición · T-144 planilla PDF NTE/DAR · T-145 exposición por establecimiento · T-146 vencimiento anual + alertas · T-147 semáforo + historial + descarga histórica, #259/#261/#262/#263/#264—). Previo (2026-06-13): RAR Fase 1 #259. Previo (2026-06-09): campo Puesto → catálogo **CERRADA** —T-129 fase B dropeó la columna legacy `empleados.puesto` + función backfill + puente, #234—; rediseño del dashboard **CERRADO** —T-131 fase A operativo #235 + fase B semáforo por cliente #238—; T-132 #236 endureció el flake E2E del guard `EXEC_NOT_DRAFT`.
...
- **RAR (Relevamiento de Agentes de Riesgo)** ✅ **ÉPICA COMPLETA** (2026-06-14): vertical nuevo —la DJ anual del 658/96 de trabajadores expuestos—, cerrado punta a punta en prod. **Fase 1** (T-143, #259): catálogo de agentes `rar_agentes` (seed de 22 agentes con códigos ESOP de la Res SRT 81/2019) + exposición, módulo `src/app/(app)/rar/`. **Fase 2** (T-144, #261): nómina de expuestos derivada de la herencia puesto→empleado + planilla PDF NTE/DAR (Res SRT 37/2010). **T-145** (#262): exposición por **establecimiento** (`cliente_puesto_agentes`, drop de `puesto_agentes`; ADR-0016 decisión B revisada). **Fase 3a** (T-146, #263): vencimiento anual en el calendario — tabla `rar_presentaciones` inmutable (snapshot legal) + tipo `rar_anual` + RPC `gen_rar_vencimiento_calendar_for` (evento + reminders [60,30,7,0]). **Fase 3b** (T-147, #264): 4ª rama `rar_anual` en `semaforo_clientes` (el RAR vencido pinta el cliente en el dashboard) + historial de presentaciones + descarga histórica del PDF desde el snapshot. Migraciones `20260613000001..04`. Modelo en `docs/adr/0016-rar-modelo-datos.md`; detalle en `operativo.md`.
