...
import { removePuestoAction } from './puestos/actions';
...
export function RemovePuestoButton({
  empleadoId,
  puestoId,
  puestoNombre,
  empleadoFullName,
    ...
    startTransition(async () => {
      ...
      switch (result.code) {
        ...
        case 'INVALID_INPUT':
          ...
          return;
        default:
          toast.error('No se pudo quitar el puesto', { description: result.message });
      }
    });
  }
  ...
  return (
    <AlertDialog open={open} onOpenChange={(next) => !isPending && setOpen(next)}>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>¿Quitar el puesto {puestoNombre}?</AlertDialogTitle>
          <AlertDialogDescription>
            Se eliminará la asignación de <strong>{puestoNombre}</strong> a {empleadoFullName}. La
